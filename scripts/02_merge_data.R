...
# 合并 NHANES PFAS C-J + P_PFAS + 26 模块 + NCHS LMF
# 输入:  data/raw/*.xpt + data/raw/mortality/*.dat
# 输出:  data/processed/nhanes_raw_merged.RData
#         (含 nhanes_all + rx_all + mort_all + cycle_tag)
#
# 关键: PFAS 跨周期文件名漂移 normalize (L24PFC_C/PFC_D-G/PFAS_H-J/P_PFAS → pfas)
#       J⊂P_ dedupe 在 03_clean_data.R 处理
# ============================================

library(haven); library(dplyr); library(purrr)

cat("========================================\n")
cat("合并 PFAS C-J + P_ + 26 模块 + 死亡链接\n")
cat("========================================\n\n")
...
merge_cycle <- function(cyc_row) {
  ...
  # NOTE (2026-06-01): 2013-14 PFOS/PFOA exist ONLY in the surplus-serum file SSPFAS_H
  # (the main PFAS_H subsample lacks them) under weight WTSSBH2Y, which is NOT poolable with
  # the main PFAS-subsample weight (WTSB2YR). So 2013-14 PFOS/PFOA are excluded BY DESIGN:
  # they remain NA here and drop out of the six-cycle PFOS/PFOA analytic frame (n ~ 7,456),
  # while PFNA/PFHxS/PFDA/Me-PFOSA-AcOH keep all seven cycles. We deliberately do NOT merge
  # SSPFAS_H, to keep the pooled-weight series internally consistent (see Supplement S2).
...
read_mort <- function(path) {
  # Official NCHS public-use Linked Mortality File fixed-width layout (48-byte record):
  # SEQN 1-6, filler 7-14, ELIGSTAT 15, MORTSTAT 16, UCOD_LEADING 17-19, DIABETES 20,
  # HYPERTEN 21, filler 22-42, PERMTH_INT 43-45, PERMTH_EXM 46-48.
  widths <- c(
    SEQN = 6, PADDING1 = 8, ELIGSTAT = 1, MORTSTAT = 1,
    UCOD_LEADING = 3, DIABETES = 1, HYPERTEN = 1,
    PADDING2 = 21, PERMTH_INT = 3, PERMTH_EXM = 3
  )
  df <- tryCatch(
